...
- Loss functions are most potent in driving the "average quality" up, but can backfire in textures that are easiest to learn, causing the loss curve to drop much slower. Contrast "Bare Bones" texture set in run20 and run33. (Better: Generate new test set that trained on Bare Bones with and without loss for better comparison.) This may be due to the fact that the various criteria added by penalty terms are always rules of thumbs, interferring with the model's learning process for tasks it otherwise already excels at.
...
Analyze the process of preprocessing ~ training ~ ensemble. Analyze the results supported by images (fix run33: the best model, see checkpoints/run33).

## 5. Conclusion

What this entails.

Future work includes generic style transfer, and automatic routing logic in the ensembler.

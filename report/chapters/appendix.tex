...
Train the full ensemble sequence for all packs:

\begin{lstlisting}[language=bash]
spritecraft train --checkpoint-dir checkpoints/final --steps 10000 --mode recolor_first
\end{lstlisting}

To train a single stage explicitly, use:

\begin{lstlisting}[language=bash]
spritecraft train --checkpoint-dir checkpoints/final --steps 2500 --mode recolor_only
spritecraft train --checkpoint-dir checkpoints/final --steps 10000 --mode std_only
\end{lstlisting}

The \texttt{recolor\_first} mode trains RecolorNet then StyleAwareUNet
sequentially for all packs and passes the recolor checkpoint directory to
diffusion validation automatically. Monitor training with TensorBoard:

\begin{lstlisting}[language=bash]
tensorboard --logdir checkpoints/final/tensorboard
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
spritecraft generate --pack Ashen_16x --checkpoint checkpoints/final --textures stone dirt oak_planks
\end{lstlisting}

Or sample randomly:

\begin{lstlisting}[language=bash]
spritecraft generate --pack Ashen_16x --checkpoint checkpoints/final --random 8
\end{lstlisting}
